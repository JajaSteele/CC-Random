local stat, err = pcall(function()
    ...
    while true do
        ...
        if last_input == 0 and new_input > 0 and config.text_on ~= "none" then
            ...
            playAudio("http://172.16.143.50/?tts="..textutils.urlEncode(config.text_on).."&voice="..textutils.urlEncode(config.voice))
        elseif last_input > 0 and new_input == 0 and config.text_off ~= "none" then
            print("Playing TTS for OFF")
            playAudio("http://172.16.143.50/?tts="..textutils.urlEncode(config.text_off).."&voice="..textutils.urlEncode(config.voice))
        end
    end
end)
